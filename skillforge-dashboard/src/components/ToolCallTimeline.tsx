    ...
    <Timeline
      ...
      items={toolCalls.map((tc) => {
        ...
        if (tc.input != null) {
          collapseItems.push({
            key: 'input',
            label: <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Input</span>,
            children: (
        ...
        if (tc.output != null) {
          collapseItems.push({
            key: 'output',
            label: <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>Output</span>,
            children: (
          ...
          children: (
            <div style={{ marginTop: -6, paddingBottom: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', lineHeight: 1.3 }}>
                <ToolOutlined style={{ fontSize: 10, color: 'var(--text-muted)' }} />
                <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-secondary)' }}>{tc.name}</span>
                {tc.duration != null && (
