...
// MULTIMODAL-MVP redesign (2026-05-14): no separate Multimodal model picker.
// Vision-capable models are tagged with a "多模态" chip in the main Model picker;
// upload UI gates on whether agent.modelId is in that set. Tests below cover
// the chip rendering on the main picker.
describe('AgentDrawer Overview — vision chip on main Model picker', () => {
  beforeEach(() => {
    warningSpy.mockClear();
    vi.mocked(updateAgent).mockClear();
  });
  ...
  it('no longer renders the obsolete Multimodal model picker', async () => {
    renderDrawer(makeAgent());
    // Picker test ids removed alongside the field. The old picker would have
    // surfaced these test ids — assert their absence to guard against accidental
    // re-introduction.
    await waitFor(() => {
      // wait for the drawer to render any model picker so we know rendering ran
      expect(screen.queryByText('Model')).toBeInTheDocument();
    });
    expect(screen.queryByTestId('multimodal-model-select')).toBeNull();
