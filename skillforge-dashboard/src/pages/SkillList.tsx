...
import {
  ...
  type SkillDraft, type SkillAbRun, type SkillVersionEntry,
...
/* ─── Skill Detail Drawer ─── */
function SkillDrawer({ skill, tab, setTab, onClose, onToggle, onDelete, currentUserId }: {
  skill: SkillRow;
  tab: string;
...
const SkillAbPanel: React.FC<SkillAbPanelProps> = ({ skillId }) => {
  const queryClient = useQueryClient();
  const { userId: currentUserId } = useAuth();
...
const SkillEvolutionPanel: React.FC<SkillEvolutionPanelProps> = ({ skillId, agentId, currentUserId }) => {
  const queryClient = useQueryClient();
  const [flash, setFlash] = useState<string | null>(null);
  const flashTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
