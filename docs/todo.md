...
| # | 任务 | 来源 | 优先级 |
|---|------|------|-------|
| P1-2 | CompactionService 解锁 LLM 调用：Full compact 持 stripe lock 调 LLM → head-of-line blocking，拆三阶段（Phase 1 guard/snapshot → Phase 2 LLM → Phase 3 persist），LLM 调用不持 stripe lock | P1 | P1 |
