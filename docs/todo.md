...
| ID | 完成日期 | Commit | 交付 |
| --- | --- | --- | --- |
| **OBS-3 跨 session unified trace tree** | 2026-05-03 | OBS-3 commit (hash 待补) | [交付索引](delivery-index.md)；M5/M6 待跟进，OBS-2 PRD 仍 active |
...
| **OBS-3 跨 session unified trace tree** | OBS-2 M3 后父子 trace 可观测但不串联：父 trace waterfall 看不到 child 在做什么，必须 SubagentJumpLink 跳到 child session。现场用 TeamCreate 派 3 个并行 child 时（session `6f18ecca`），需要点 4 次跳转才能看完 + 难看到"父等待 N 个 child"的整体时间线 | OBS-2 M3.5 / M4 / M5 / M6 全交付后 / 用户开始大规模用 TeamCreate 多 agent 协作场景 | 主旨：**选项 A（用户已拍板）— UI 层 unified trace tree**，schema 不动。后端新 endpoint `GET /api/traces/{traceId}/with_descendants` 递归 (DFS via `t_session.parent_session_id`) 找 descendants + 聚合 parent + descendant spans 标 depth；前端 waterfall nested rendering（TeamCreate row 下挂 child sub-tree，缩进 + 折叠按钮）+ child status badge（running/ok/error/cancelled）+ child sub-tree 时长 = child trace 真实 duration（不是派发耗时）。child 仍可通过 SubagentJumpLink 独立打开。性能：descendant 数 ≤10 同步加载，超过 lazy load。**Mid 档**，需求包真做时建。**已锁决策**：拒绝选项 C（跨 session 共享 trace_id 会破坏 tech-design §9.2 child session 独立性不变量） |
