...
| **WECHAT-CHANNEL** | 加微信 channel（现仅飞书）。SkillForge channel 适配器架构现成（feishu/telegram/mock），加平台无 migration。**待拍路线**：A 企业微信自建（推荐，契合现有架构，无扫码）/ B openclaw-wechat bridge（扫码+个人微信但借壳+额外 Node 进程）| prd-draft（待拍路线） | [需求包](requirements/active/2026-06-18-WECHAT-CHANNEL/index.md) | [MRD](requirements/active/2026-06-18-WECHAT-CHANNEL/mrd.md) | 待路线定 | 待路线定 | — |
