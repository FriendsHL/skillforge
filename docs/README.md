...
| **TASK-RESUME-ON-RESTART** | 任务断点续存：重启后在跑的任务（根 agent-loop / workflow / ACP cc-codex）不静默丢失/卡死。现状=子 agent 已能 resume，根会话被标 error、workflow 卡 running。推广子 agent 恢复范式 + 统一重启对账。红灯 Full（核心文件）。来源用户 2026-06-24 | backlog | [需求包](requirements/backlog/TASK-RESUME-ON-RESTART/index.md) |
