...
| 多用户 / 权限模型 design doc | 当前 auto-token 是单用户假设；P11 跨 Agent 调用身份、P12 定时任务触发 session 的身份都会踩到边界。 | 写 design doc，明确 agent / session / memory / scheduled_task 如何隔离；不要求立刻实现。 |
