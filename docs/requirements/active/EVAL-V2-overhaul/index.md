...
- M3a-b1 origin 字段 / 5 过滤 / 3 spawn 复制 **已交付**
- M3a-b2 task/item 双写 + migration 收尾 **已完成，待提交**
- M3b Dataset UI 完整化 **已交付**（commit `b349f6f`）
- M3c 归因闭环 **已完成，待提交**（三种 analyze 入口 + `t_eval_analysis_session` + `AnalyzeEvalTask` 写回通道）
- M3d Compare runs **已完成，待提交**
- M3e Case → trace **已完成，待提交**
- M3f Annotation queue **已完成，待提交**（队列 / 持久化 / pending→applied，并接上“从 corrected expected 创建新版本”）
- M3g Dataset versioning **已完成，待提交**（scenario lineage + version selector/history drawer）
- M5 Trace 数据回流 dataset **已交付**（commit `2d10edf`）
- M4 Score 多维度 **已完成，待提交**
- M6 Apply suggestion 接 PromptImprover **已完成，待提交**
- **M 系列（M4 + M5 + M6）已完成**
- 下一步：**统一测试后提交 M4 + M6，并归档 EVAL-V2**
