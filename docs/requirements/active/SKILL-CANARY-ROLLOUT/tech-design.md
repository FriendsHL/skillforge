...
| 文件 | 改动 | 不变量保护 |
|---|---|---|
| `skillforge-core/.../engine/AgentLoopEngine.java` | spawn skill 之前加 `CanaryAllocator.allocateSkillVersion(...)` 一处调用 | persistence-shape-invariant.md + identity-column-on-rewrite.md 两条 Iron Law 不变；reviewer 显式 grep diff 验证 ChatService 持久化 Message 字节 + SessionService.rewriteMessages 路径完全没动 |
...
```java
public Long allocateSkillVersion(String sessionId, Long agentId, Long baselineSkillId) {
    ...
    Optional<CanaryRolloutEntity> activeCanary = canaryRepository
        .findActiveCanary(agentId, "skill");
    if (activeCanary.isEmpty()) {
        return baselineSkillId;  // 一刀切默认路径
    }

    CanaryRolloutEntity canary = activeCanary.get();
    int pct = canary.getRolloutPercentage();
    if (pct == 100) {
        return canary.getCandidateVersionId();  // 100% 已上线
    }
    if (pct == 0) {
        return baselineSkillId;  // rolled_back 或刚 reset
    }
    ...
    if (existingGroup.isPresent()) {
        return parseSkillIdFromGroupValue(existingGroup.get());
    }

    // 3. 新 session → hash 分流
    int bucket = (sessionId.hashCode() & 0x7FFFFFFF) % 100;
    Long pickedVersion = bucket < pct ? canary.getCandidateVersionId() : baselineSkillId;
...
- [ ] **Phase 1.0：证伪 + 红测试**
  - 跑 grep AgentLoopEngine spawn skill 入口位置
  - 单元测试 SkillEntity 加 2 列后 JPA 不破现有
  - V1 t_session_annotation 复用 canary_group 值校对
  - 写红测试 CanaryAllocatorRedTest @Disabled
