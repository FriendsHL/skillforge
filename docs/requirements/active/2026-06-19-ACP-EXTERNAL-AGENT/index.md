...
- **L1+L3（测试门 + 建 PR，prompt 驱动）**（2026-06-20，`V161`）：不写 SkillForge 编排，靠 claude-code persona 指令让 cc 自己「改完 → 跑 build/test → commit → push 分支 → 绿则 `gh pr create`（带 diff + 测试结果）→ 报告 PR URL」。**验证**：smoke run 证明 cc 在 auto 权限模式、worktree 里能 `git push -u origin HEAD`（分支落 origin）+ 跑 `gh`（exit 0），无需改权限模式。**取舍**：L1 是 cc **自报**绿，v1 的真门是**人审 PR**；SkillForge 侧**独立硬门**推迟（只在 L4 无人回路自动部署时才需要，用户「硬门再说，不想太定制化」）。**base 策略（已定）**：`worktree-base-ref` 默认 `origin/main`，cc 每次基于最新远端 main（创建前 best-effort fetch）出分支 → PR diff 干净 vs main（gh 默认 base=main）。**live E2E**：cc（claude-code persona，session agent=23）基于 origin/main 改 README → push → 干净 PR `#3`（base=main，+1/-0，独立 gh 验证 OPEN）。
- **AC-6 codex 接入**（2026-06-20，`V162`）：adapter 从全局单一改成**按 agent model_id 选**（`acp:<key>` → `skillforge.acp.adapters[key]`：claude-code→`@agentclientprotocol/claude-agent-acp`、codex→`@zed-industries/codex-acp`）；`AcpClientFactory.create` 加 adapter 入参；非 acp agent 退回默认 adapter。cc 专属 OTLP 遥测 env 收窄到仅 cc adapter（codex v1 只 ACP session 级可观测）。worktree + buildCcPrompt + L1/L3 全复用，V162 seed codex agent(id=24, acp:codex)。**live E2E**：codex 真在基于 origin/main 的 worktree 跑（ps 抓到 `@zed-industries/codex-acp-darwin-arm64/bin/codex-acp` 进程 + ~/.codex sqlite 活动 = 硬证据启的是 codex 非 cc），读到 origin/main README，未改文件。**已知 nit**：worktree 分支名仍用 `acp/cc-` 前缀（codex 也叫 cc-），cosmetic 未改。
