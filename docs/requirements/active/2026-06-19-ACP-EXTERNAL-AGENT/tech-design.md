...
### 候选路径（Plan 阶段定）
- **B1 适配层**（最小）：保留现有 LlmSpan，加一个 **OTLP receiver** 把进来的 OTel span 翻译成 LlmSpan/或并存的通用 span 表。改动小，但仍非「真 OTel」，通用 span 语义有损。
- **B2 双写**：SkillForge 自身 instrumentation 同时产 OTel（OpenTelemetry Java SDK）+ 现有 LlmSpan，逐步迁移读路径到 OTel store。过渡平滑，双维护成本。
- **B3 转真 OTel**（用户倾向）：引入 OpenTelemetry Java SDK，SkillForge span 用通用 OTel span（gen_ai 语义约定）；OTLP receiver 接外部；trace 视图读 OTel store；现有 LlmSpan 作为 gen_ai span 的子集/迁移。最干净、最大改动。
...
## 开放问题（Plan / 用户决策）
- OQ-1：Track B 走 B1/B2/B3 哪条？（用户倾向 B3 真 OTel，但范围最大）
