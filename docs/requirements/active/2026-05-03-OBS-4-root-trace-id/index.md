...
updated: 2026-05-03
milestones_done: -
milestones_pending: M0 (schema), M1 (write path), M2 (read API), M3 (FE 二级折叠瀑布流), M4 (观察 + 收尾)
---
...
| 里程碑 | 范围 | 工期估算 | 状态 |
|---|---|---|---|
| M0 | schema migration（V44 加 `root_trace_id` + `active_root_trace_id`）+ 历史数据 `root_trace_id = trace_id` | 0.5 天 | ⏳ |
| M1 | 写入路径（AgentLoopEngine / ChatService / TeamSendTool / SessionEntity 四处） | 2-3 天 | ⏳ |
| M2 | Read API（`GET /api/traces/{rootId}/tree` 返回全部 traces + spans） | 1 天 | ⏳ |
| M3 | FE 瀑布流二级折叠 inline group 渲染 | 3-5 天 | ⏳ |
