...
| D2 | failing-scenario grounding 来源 | ✅ **已 ratify(2026-06-03)：(a) benchmark baseline FAIL 场景**（现成 36 场景里 agent 判 FAIL 的那些当 grounding，可复现、最低成本）。(b) session 失败转可复现场景留后续扩展。|
| D3 | report facets 丰富度 | 按我们数据量裁剪几维（不照搬 55）|
| D4 | benchmark 范围 | 先用现有 36 场景当 north-star，dogfood 增量扩 |
| D5 | 分期顺序 | ✅ **已 ratify(2026-06-03)：P2 对靶先**（G1 scenario-grounding + G4 MULTIPLE TIMES 先做，让赢家先是真提升）→ P1 闭环采纳 → P3 benchmark 贯穿。理由:现在候选多平手/噪声(实测 tool-budget 75→75)，先给采纳按钮没东西可采。|
| D6 | P2 内部切分 | ✅ **已 ratify(2026-06-03)：P2 拆两个 Full 子期**——**P2-a = G4**（report 对靶：MULTIPLE TIMES + recurrence facets，让 aggregator 挑对靶子）**先做** → **P2-b = G1+G2**（候选 grounding 接 benchmark FAIL 场景 + trace re-hydrate，让候选基于真失败改）。G3 predicted_impact 排 P2-b 之后。逻辑顺序：先挑对靶子再改。|
