...
- [x] 确认 `ToolResultRetrieveSkill` 是否放进首个 PR，还是作为 follow-up。**2026-04-30 决议：follow-up**；首版不交付，与 Claude Code `applyToolResultBudget` 设计一致（preview 对模型即是最终视图）；上线后若有明确诉求再开 P9-2-follow-up 单独评估。已写入 PRD 非目标。
