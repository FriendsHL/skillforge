...
> 更新于：2026-04-28
> **归档说明**：本文是重整前 ToDo 原文备份，保留历史上下文和旧路径引用；当前入口请读 [../README.md](../README.md) 和 [../todo.md](../todo.md)。
...
> **BUG-G 收尾**：根因 BUG-G-2 已修（2026-04-28）；BUG-G-1 / BUG-G-3 防御性子项**暂缓**，再遇到 dangling tool_call 类事故时重启（2026-04-28）
> **OBS-1 决策细化**：MVP 收窄到 OBS-1-1+2+3；独立 maven module `skillforge-observability`；Observer 模式（不用 AOP）；Q3 选 A3 ETL + `source` 字段；ETL 评估完成（40% 老字段拿不到）；Q1/Q2/Q4-Q8 草拟待最终拍板（2026-04-28）
...
> 本文先看「当前队列」，历史长复盘保留在各章节和底部「已完成」表格中，避免把已完成事项误判为待办。
...
| 顺序 | 状态 | 内容 | 预估 | 核心判断 |
| --- | --- | --- | --- | --- |
| 1 | **🔍 OBS-1** | Session × Trace 合并视图 + LLM call 完整 payload 落库 + UI Raw Viewer + 压缩验证视角 | 待估期 | 用户需要直接看完整 LLM request body 判断压缩 / tool_calls / provider 兼容问题；与 P15 Tool 层裁剪互补 |
| 2 | **Sprint 3** | P9-2 长对话 tool 归档（独立 PR） | ~2 周 | 触碰 `CompactionService` 核心文件，必须 Full Pipeline；解决真实长 session 慢性 context 膨胀 |
| 3 | **⚠️ Sprint 4 前置** | Cost Dashboard · PG 备份 · 多用户权限 design doc | 决策先行 | P12 开工前必须有答案，否则 token 成本、数据备份、身份边界都会放大风险 |
| 4 | **Sprint 4** | P12 定时任务（收窄首版） | 3-4 周 | user 型调度最小集；SystemJobRegistry 与高级可靠性推迟到 V2 |
| 5 | **Sprint 5** | P9-4 Partial compact + P9-5 Post-compact 恢复 | 按需 | P9-5 依赖 P9-4；需先写 design doc 明确"最近文件"数据来源 |
| 6 | **Sprint 6** | P10 聊天斜杠命令（收窄 4 条） | 5-8 天 | `/compact` 触碰核心 compaction 路径，需 Full Pipeline；`/model` 只改 session 级 |
| 穿插 | **🔒 安全** | SEC-1 Channel 配置 AES-GCM 加密 | 1-2 天 | 当前单机/本地使用暂缓；多端部署或 P12 正式上线前重评 |
| V2 | **推迟池** | P14 · P3-2/4 · P15-3/6 · P12-3/6 · P10-4/5 · P13-9 · BUG-G-1/3 防御性子项 | 推迟 | 见底部 V2 推迟池 |

### 近期待归档完成项

| 项 | 完成时间 / 证据 | 备注 |
| --- | --- | --- |
| BUG-G-2 toolUseBlocks 与 stopReason 一致性修复 | 2026-04-28 | qwen 流式 id/name 防覆盖 + `LlmResponse.isToolUse()` 结构优先 + `AgentLoopEngine` 使用过滤后有效 tool_use blocks；BUG-G-1/3 暂缓至 V2 |
| ENG-1 · ENG-2 · P9-5-lite | 2026-04-23 | 由 session `9347f84c` 事故触发，已作为紧急修复完成 |
| Compact Breaker 误触 + LLM Stream 抗抖 | 2026-04-24 commit `121e8dc` | BUG-A/B/C/D/E/E-bis 完成 |
| thinking-mode-v1 | 2026-04-25 commit `55969db` | per-agent thinkingMode + reasoningEffort + provider 协议感知 |
| SEC-2 Hook Source Protection + Agent-authored Hook Binding V1 | 2026-04-25 | 三源隔离 + Hook 查询 / 提案 / 审批闭环完成 |
| BUG-F Compact 摘要存储重构 | 2026-04-26 commit `e9b48f3` | DeepSeek `Duplicate value for 'tool_call_id'` 根因修复 |
| Sprint 1 零依赖防腐 | 2026-04-26 | P9-7 / P3-1 / P3-3 / P13-3 / P13-4 完成 |
| Sprint 2 P11 + P15 | 2026-04-26 | AgentDiscovery / SubAgent / GetTrace / GetSessionMessages / GetAgentConfig / UpdateAgent 完成 |
| Memory v2 | 2026-04-27 | MEM-1 ~ MEM-5 主链路完成；PR-5 本地已验证，待后续提交 |
| P1 Skill 生命周期统一（方案 C） | 2026-04-28 commit `555bdba` | Skill Control Plane 完成；同 PR 合并 BUG-G-2 |
| DEBT-1 SkillList.tsx 拆分 | 2026-04-23 commit `0576213` | 原 47K 单文件已拆分 |
...
### ~~🔥 BUG-G dangling assistant tool_call 防腐~~ ✅ 根因已修（2026-04-28）— BUG-G-1/3 防御性子项暂缓
...
| **BUG-G-1** OpenAI-compatible 发送前历史防腐（方案 3）→ V2 | 防御性兜底：在 `OpenAiProvider.convertMessages` 或前置 sanitizer 校验 assistant `tool_calls` 配对完整性，发现缺失时降级或补 synthetic tool。**根因 BUG-G-2 已堵源头，暂缓**；再遇到历史 dangling 触发 provider 400 时重启 |
| **BUG-G-3** 失败路径持久化保护 → V2 | 防御性兜底：runLoop error/cancel/max-loop 等异常路径，最终落库前校验尾部 assistant tool_use 配对，缺失时补 synthetic error tool_result 或降级文本。**暂缓**；再遇到异常退出污染 DB 触发后续 session 卡死时重启 |
...
### 🔍 OBS-1 Session × Trace 合并 + Trace 详情完整化（2026-04-28 新增；2026-04-28 决策细化）
...
**首版做 OBS-1-1 + OBS-1-2 + OBS-1-3**；OBS-1-4 / OBS-1-5 推迟到首版上线、用户用起来后再做（这两项 UI 形态硬猜会做错——需要先看到真实 payload 长什么样、用户怎么用 Raw Viewer 才能定准 UI 形态）。

| 子任务 | 说明 | MVP |
| --- | --- | --- |
| **OBS-1-1** Session × Trace 统一视图 | 改造现有 **Sessions 页**（**Q5**）：点 session 进入合并视图，左侧消息时间线（user / assistant / tool_use / tool_result），每条消息可展开下挂的 LLM call / Tool exec span，按时间对齐；保留现有 Traces 页作为 trace 维度入口 | ✅ |
| **OBS-1-2** LLM span 完整 payload 落库 | 新建 `t_llm_trace` + `t_llm_span` 两表（PG，参考 Langfuse / Opik schema），不在原 `t_trace_span` 上扩；DB row **永远只存 32KB 截断摘要 + `*_blob_ref`**（**Q2 都落 blob，统一路径，UI 体验一致**）；流式调用同时落 raw SSE 流 + 累加后 JSON 两份（**Q1**）；`usage` / `cost` 用 JSONB 字段，命名抄 OTel `gen_ai.usage.*` semantic convention；写入与 query 全部走新增的 `LlmTraceStore` interface | ✅ |
| **OBS-1-3** UI Payload Viewer | Span 详情面板新增"Raw Request / Raw Response" tab：默认显示 32KB 摘要，按"看完整内容"按钮才拉 blob；JSON 折叠 + 关键词搜索 + 一键复制 + size badge；超过阈值（如 1MB）走分块流式渲染防卡顿；脱敏仅 HTTP headers 层剥离（**Q7**：`Authorization` / `x-api-key` / `Bearer *` / `api-key` / `apikey`），request body 不主动扫（SkillForge 现状 body 不带 secret） | ✅ |
| **OBS-1-4** 压缩验证视角 | 对 `compact` 类 LLM call（summary 生成）在 UI 标记特殊徽章；可对比"压缩前消息列表"vs"压缩后实际发给模型的 messages" | ⏸ V2 |
| **OBS-1-5** Provider quirk 诊断信号 | 已知问题（reasoning_content 缺失、tool_call_id 重复、dangling tool_use）在 UI 自动检测红字标注 + 文档锚点 | ⏸ V2 |

#### 决策清单（2026-04-28）

| # | 决策点 | 选择 | 状态 |
| --- | --- | --- | --- |
| Q1 | 流式 SSE 落库形态 | C 两份都存：累加后 JSON 落 DB（默认看）+ raw SSE 落 blob（按需调 chunk 边界 bug） | 草拟 |
| Q2 | blob 触发阈值 | B 都落 blob：DB 永远只存 32KB 摘要 + blob_ref 永远填，UI 体验一致 / DB row 体积可预测 | 草拟 |
| Q3 | 旧 `t_trace_span` 处理 | **A3 一次性 ETL + `source` 字段标记**（消除 fallback if 分支，代码路径单一） | ✅ 已定 |
| Q4 | trace_id 体系 | A 复用现有 trace_id（OBS-1-1 统一视图 JOIN 干净；保持 SkillForge 全链路约定） | 草拟 |
| Q5 | UI 落位 | B 改造现有 Sessions 页：点行进入合并视图；Traces 页保留作 trace 维度入口 | 草拟 |
| Q6 | 保留期 + 清理 | 30 天 + cron 每日清理过期 blob 和 DB row；配置项 `observability.llm-trace.retention-days=30`，清理失败 log warn 不抛 | 草拟 |
| Q7 | 脱敏 | 仅 HTTP headers 层剥离；request body 不主动扫 | 草拟 |
| Q8 | SubAgent 展示 | 跳转：父 session 视图里 SubAgent 调用 span 显示"→ 子 session #xyz"，点击跳转；嵌套树 V2 做 | 草拟 |
| 其他 | 物理隔离 | **独立 maven module `skillforge-observability`**，跟 core / tools / server 同级（compile time enforce 解耦边界） | ✅ 已定 |
| 其他 | LlmProvider 周边 | **Observer 模式**（不用 AOP）；详见下方"为什么不用 AOP" | ✅ 已定 |
| 其他 | PG backup 是否带 payload | 不带：payload 文件不在 PG 里、PG auto-restore 是极小概率应急路径；丢失只影响 raw 查看，DB 摘要仍可用 | ✅ 已定 |
| 其他 | payload 目录结构 | `data/llm-payloads/{yyyy-MM-dd}/{traceId}/{spanId}-{request\|response}.json`（按天分目录便于清理） | ✅ 已定 |

#### 物理隔离 + 落地方案

**新增独立 maven module `skillforge-observability`**（跟 core / tools / server 同级）：

```
skillforge/
├── skillforge-core                 # 加 LlmCallObserver interface（仅接口，无实现）
├── skillforge-tools
├── skillforge-observability        # 新模块
│   ├── api/
│   │   ├── LlmTraceStore           # 接口
│   │   └── BlobStore               # 接口
│   ├── impl/
│   │   ├── PgLlmTraceStore         # PG 实现
│   │   ├── FileSystemBlobStore     # 文件系统 blob 实现
│   │   └── TraceLlmCallObserver    # 实现 LlmCallObserver
│   ├── entity/
│   │   ├── LlmTraceEntity
│   │   └── LlmSpanEntity
│   ├── etl/
│   │   └── LegacyLlmCallMapper     # ETL 字段映射 helper
│   └── migration/
│       ├── V25__create_llm_trace_tables.sql
│       └── V26__migrate_legacy_llm_call.sql
├── skillforge-server               # 依赖 observability，自动收集 observer 注入到 provider
├── skillforge-dashboard
└── skillforge-cli
```

**依赖方向**：core 定义接口（无实现）→ observability 依赖 core 拿类型 + 提供实现 → server 依赖 observability + core，启动时 Spring 自动收集所有 observer 注入到 provider。core ←—— observability ←—— server，无循环依赖。
...
// OpenAiProvider / ClaudeProvider 改动最小化
public OpenAiProvider(..., List<LlmCallObserver> observers) {
    this.observers = observers != null ? observers : List.of();
}

private void notifyBefore(LlmCallContext ctx) {
    for (var o : observers) {
        try { o.beforeCall(ctx); }
        catch (Exception e) { log.warn("observer failed", e); }  // 不影响主链路
    }
}
```

**为什么不用 AOP**（评估结论，避免未来重新争论）：
1. **流式回调让 AOP 零侵入承诺破产**：`chatStream(req, handler)` 方法返回时 SSE delta 还在异步收，AOP `@Around` 拿不到完整 response，必须 wrap handler——跟 Observer 一样要 wrap，没省事
2. **ThreadLocal 传播是显式工程**：OkHttp 异步回调线程 + `@Async` SubAgent 派发都不会自动跨 ThreadLocal，要手动 capture/restore；既然要手动传播，不如直接传 LlmCallContext 参数
3. **业务上下文哪儿来**：AOP 切面拿不到 sessionId / agentId，必须有人在调用前先 set ThreadLocal——这个人是 AgentLoopEngine，那不如它直接调 recorder
4. **Spring AOP 对工厂方法 new 的 bean 不生效**：thinking-mode-v1 commit `55969db` 在 `SkillForgeConfig.llmProviderFactory` 用工厂方法包了 try-catch；要么改成 @Component，要么换 AspectJ 编译期织入（复杂度跳一档）
5. **AOP footgun 跟 @Transactional self-invocation 叠加**：CLAUDE.md 第二条 footgun 已经踩过，再叠一层 trace AOP 只会更难调试

#### 主链路保护（design doc 必须写明的硬约束）

1. Observer 调用一律 try-catch，trace 失败只 log，绝不抛给业务
2. Blob 落盘走 `@Async` executor，主线程只写 DB row（DB 失败也不阻塞 LLM 调用）
3. Payload 序列化失败兜底：写空 JSON + log warn，不让序列化 bug 干掉 LLM 调用
4. Executor 队列满时直接 drop + 计数器告警，不阻塞业务也不无界堆积

#### A3 ETL 关键设计

**评估结论（2026-04-28 Explore agent 扫码确认）**：
- t_trace_span **没有 attributes JSONB**（V1 init 18 列固定 schema）
- LLM_CALL row 在 `AgentLoopEngine.java:745-799` 写入
- input / output 列只存**摘要**（`"messages: N | last_user: [preview]"` / 响应文本 + tool_use 列表），**不是完整 payload**
- **40% OBS-1 期望字段拿不到**：完整 input/output / cache_read_tokens / usage_json / cost / finish_reason / request_id / reasoning_content / error_type 全部缺失
- 失败 LLM 调用甚至**没写 LLM_CALL row**（异常直接中断 loop）
- ETL 可行性：**中**——关联字段（trace_id / session_id / model / tokens / latency）齐全，内容字段稀缺

**形式**：Flyway migration 一把梭

```
V25__create_llm_trace_tables.sql      -- 建 t_llm_trace + t_llm_span 空表 + indexes + unique constraint
V26__migrate_legacy_llm_call.sql      -- ETL：含 resolve_provider_from_model_id PG plpgsql function
```
...
| provider | resolve_provider_from_model_id(model_id) | PG plpgsql function 解析前缀（claude-* → claude / gpt-* → openai / deepseek-* → deepseek / qwen-* → qwen / 兜底 unknown） |
| input | t_trace_span.input | 摘要，非完整 payload |
| output | t_trace_span.output | 摘要，非完整 payload |
| input_blob_ref | NULL | 老 row 无 raw blob（物理事实） |
| output_blob_ref | NULL | 同上 |
...
| error | t_trace_span.error | 通常为空（失败 row 没写） |
| **cache_read_tokens / usage_json / cost_usd / finish_reason / error_type / request_id / reasoning_content** | **NULL** | **物理拿不到，ETL 也变不出来** |
| **source** | **'legacy'** | **新表加 `source VARCHAR(16) NOT NULL DEFAULT 'live'` 列；'live' 和 'legacy' 二选一** |

**`source` 字段消除 if 分支（核心设计）**：

UI 端用 enum `LlmSpanSource.LIVE / LEGACY` 做展示降级（数据驱动，不是路径分叉）。完全没有 `if (blob_ref != null)` 这种判断，最多有 `if (span.source == LEGACY)` 这种合理的业务展示分支：

```tsx
{span.source === 'legacy' ? (
  <Button disabled tooltip="历史 session，无 raw payload">看完整内容</Button>
) : (
  <Button onClick={loadBlob}>看完整内容</Button>
)}
```

**幂等性**：unique constraint `(trace_id, span_id)` + `ON CONFLICT DO NOTHING`，重复跑安全

**老表清理欠债（必须写进 design doc）**：t_trace_span LLM_CALL row **先保留 3-6 个月**作为回滚安全网；ETL 跑出 bug 可以重跑，删了就回不来。3-6 个月稳定后写一个 V27 migration 批量删。**这条不写 design doc 未来交接的人不知道**。

**ETL 风险（Plan reviewer 重点 review）**：
- **数据量未知** → Plan 阶段先 `SELECT count(*) FROM t_trace_span WHERE span_type='LLM_CALL'` 验数；**超 50 万行降级到异步 ETL service**（避免 Flyway 启动卡几分钟）
- **provider 解析漏新 case** → resolve_provider_from_model_id 函数要好维护；未来加 Gemini 等新 provider，老数据 provider 字段为 'unknown'，允许 ad-hoc 写 `UPDATE` 修正
- **session 已删** → LEFT JOIN 后 agent_id 为 NULL，UI 显示 "agent unknown"
- **失败 row 缺失** → 老 session 的失败 LLM 调用无法重建，新数据从 OBS-1 上线后开始记录失败 LLM_CALL
- **测试** → **必须在跟生产数据量相当的环境跑一次**，验证 ETL 时间 + 字段正确性 + 抽样 spot check

#### 存储抽象层（OBS-1-2 必带）

所有 trace 写入 / 查询走 `LlmTraceStore` interface，**禁止 controller / service 直接写 SQL** 或暴露 ResultSet / JSONB / SQL 片段。本期只交付 `PgLlmTraceStore` 实现；未来切 ClickHouse 时新增 `ClickHouseLlmTraceStore` impl + 一次性 ETL 即可，不动业务代码。
...
当前用 **PostgreSQL（embedded zonky-PG 14）**，不引 ClickHouse。理由：
...
**设计参考**：`research-docs/llm-trace-storage-research.md`（Langfuse / Opik / LangSmith / Coze Loop 4 家 schema 对比 + OTel `gen_ai.*` semantic convention 字段命名依据；legacy todo 中保留原始引用）

#### 不在本次范围

- OBS-1-4 / OBS-1-5（推迟到首版上线后做）
- 跨 session 全文检索 / 流量重放 / 自动告警 → V2
- LLM call 录制为 eval scenario → 与 P14 协同时再考虑
- t_trace_span LLM_CALL row 物理删除 → V27 migration（3-6 个月后）

#### 与 P15 关系

P15 GetTraceTool 仍保留裁剪输出给 Agent 用（节省 token）；OBS-1 的完整 payload 是给**人**看的旁路通道，二者不互相替代

#### 开工前置（Plan 阶段必做）

- [ ] 确认 Q1 / Q2 / Q4 / Q5 / Q6 / Q7 / Q8 草拟决策（用户最终拍板）
- [ ] `SELECT count(*) FROM t_trace_span WHERE span_type='LLM_CALL'` 验生产数据量
- [ ] Full Pipeline 走 TeamCreate → Plan 对抗循环（OBS-1 是新基础设施，schema + 接口设计需要 reviewer 把关）
- [ ] Plan 阶段强制要求"先 dry-run 输出 row count + 抽样 spot check 再 commit migration"
...
| **BUG-G-1 OpenAI-compatible 发送前历史防腐** | 根因 BUG-G-2 已堵源头；防御性子项，单独投工程不划算 | 再遇到历史 dangling 触发 OpenAI-compatible provider 400 |
| **BUG-G-3 失败路径持久化保护** | 同上，防御性兜底 | 再遇到异常退出后坏尾巴写入 DB 导致 session 卡死 |
