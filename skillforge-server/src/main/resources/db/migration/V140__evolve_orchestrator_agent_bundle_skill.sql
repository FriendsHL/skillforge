-- V140__evolve_orchestrator_agent_bundle_skill.sql
--
-- AUTOEVOLVE-AGENT-LEVEL-BUNDLE Phase 4 — widen the agent-level orchestrator seed
-- to the THIRD surface: skill. The bundle pointer tuple becomes 3-wide
-- {promptVersionId, behaviorRuleVersionId, skillDraftId}; a candidate can now
-- co-change any of prompt / behavior_rule / skill. Everything else is unchanged
-- from V139 (whole-agent A/B, one overall score + target/general dual-criteria,
-- the dual gate vs-best floor + vs-original general anchor, reflection, never
-- Promote). Skill carry-forward uses baseDraftId (the skill analog of
-- basePromptVersionId / baseVersionId). Idempotent UPDATE keyed on name;
-- system_prompt-only (tool_ids/config untouched). Supersedes V139.
...
【第一轮 —— currentBestGlobal 还为空（先测原版 baseline + 拿 originalGeneral）】
  ...
  b1. TriggerAbEval(surface="agent", candidateBundle=<候选包 json>, baselineBundle={"promptVersionId":null,"behaviorRuleVersionId":null,"skillDraftId":null}, evolveRunId=<evolveRunId>) —— **不传 cachedBaselineScore**（第一轮真跑两臂，baseline=原版）。拿 abRunId。
...
【之后每一轮 —— currentBestGlobal 已有（基于 best 包累积爬 + 反哺上轮评测，baseline 不再重测）】
  ...
  dN. TriggerAbEval(surface="agent", candidateBundle=<候选包>, baselineBundle=<currentBest 包>, **cachedBaselineScore=<currentBestGlobal>**（**必传**，A/B 只跑候选臂、复用 best 分当 baseline，绝不重测）, evolveRunId)。拿 abRunId。
