-- V144__evolve_orchestrator_harvested_target.sql
--
-- AUTOEVOLVE-CLOSE-LOOP Phase BC-M2b — wire the harvested "错题本" (bad-case)
-- scenarios into the evolve-orchestrator loop. Two changes, both idempotent
-- (full re-set keyed on name; safe to re-run / flyway-repair). Supersedes V140
-- (whose only change was the system_prompt; V140's prompt body is reproduced
-- verbatim below with one added discovery step + per-round target wiring).
--
-- 1. tool_ids / config: add ListActiveHarvestedScenarios (read-only lookup of the
--    agent's ACTIVE harvested scenario ids) to BOTH the top-level tool_ids column
--    (V81 column-based filtering) AND the config JSON tool_ids (ChatService
--    runtime allowlist) — both must stay in sync (see V131 note). Everything else
--    in config is V131's value, unchanged.
-- 2. system_prompt: add a discovery step that calls ListActiveHarvestedScenarios
--    and, when non-empty, threads the ids as the explicit target subset of every
--    round's TriggerAbEval(surface=agent) (target = harvested 错题, general =
--    benchmark, measured together). Empty → normal role-based split, don't stop.
--
-- BLIND-TEST CRITICAL: the added prompt text only uses discover / target /
-- measure verbs. It forbids the orchestrator from activating any draft (that is
-- the human gate) and from inferring "how to fix" — it does not name any remedy
-- and does not even name an error signature (the oracle scores recurrence).

UPDATE t_agent
SET updated_at = NOW(),
    tool_ids = '["RunWorkflow","GenerateCandidate","TriggerAbEval","GetAbResult","RecordIteration","PromoteCandidate","ListActiveHarvestedScenarios"]',
    config = '{"maxTokens": 8192, "temperature": 0.0, "execution_mode": "auto", "tool_ids": ["RunWorkflow","GenerateCandidate","TriggerAbEval","GetAbResult","RecordIteration","PromoteCandidate","ListActiveHarvestedScenarios"]}',
